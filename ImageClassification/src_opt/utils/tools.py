...
def get_noiseword():
    NoiseWord = ["0_NonIID", "1_LongTail", "2_LabelNoise", "3_LabelNoise2", "4_DataNoise", "5_GradientNoise", "6_RandomAttack", "7_ReverseGradient", "8_ConstantAttack", "9_scalingAttack", "10_gaussian_noise"]
    return NoiseWord
...
def get_dataset(args):
    ...
    if args.dataset == 'cifar':
        ...
        # test_dataset_all[1][1]
        l = []
        for i in range(len(test_dataset_all)):
            if test_dataset_all[i][1] == 0 :
                l.append(test_dataset_all[i])
    

        # for i in test_dataset_al
        test_dataset, valid_dataset = torch.utils.data.random_split(l, [800, 200])
        ...
        if args.iid:
            ...
            print("am i here toosl, else non-iid!!!")
            # Long-tailed
            if args.noise == 1:
                user_groups = cifar_longtail(train_dataset,  args.num_users, args.noiselevel)
            # Sample 0_NonIID user data from Mnist
