...
class Shapley():
    ...
    def get_acc(self, index, left_length):
        ...
        left_acc, current_loss = test_inference(self.args, self.global_model, self.valid_dataset)#inference evaluate the given model on test dataset, calculate the accuracy, loss, return values for evaluation purposes
    ...
    """
    def eval_exactshap(self):   #This binary incrementation process ensures that every possible combination of the 10 clients is represented and evaluated.
        n = len(self.local_weights)

        def enum(l):#The enum function is a systematic way to explore all subsets (coalitions) of a set (the clients) by iterating through the binary numbers that represent these subsets. 
    ...
    #     n = len(self.local_weights)  # Number of clients

    #     # Function to iterate through all subsets of clients
    #     def enum(l):
    ...
    #     coef = np.zeros(n)  # Coefficients for each subset size
    #     fact = np.math.factorial
    #     l = np.zeros(n, dtype=int)  # Subset representation

    #     # Calculate coefficients based on the number of permutations
    #     for s in range(n):
    #         coef[s] = fact(s) * fact(n - s - 1) / fact(n)

    #     # Iterate through all subsets to calculate contributions
    #     enum(l)
    #     while np.sum(l) != n:  # Ensures we cover all subsets including the full set
    #         idx = [i for i, val in enumerate(l) if val == 1]
    #         test_weights = [self.local_weights[i] for i in idx]
            
    #         # Aggregate weights for the current subset
    #         test_weight = average_weights(test_weights)
    #         self.global_model.load_state_dict(test_weight)
    #         self.global_model.eval()
            
    #         # Evaluate the model with the current subset of clients
    #         current_acc, _ = test_inference(self.args, self.global_model, self.valid_dataset)
            
    #         # Update Shapley values based on the current coalition
    #         for i in idx:
    #             shapley_values[i] += coef[len(idx) - 1] * (current_acc - self.init_acc)
            
    #         enum(l)  # Move to the next subset

    #     # Normalize Shapley values to ensure they sum up to the total improvement
    #     shapley_values = shapley_values / np.sum(shapley_values) * (current_acc - self.init_acc)
