...
class CNNCifar(nn.Module):
    def __init__(self, args):
        super(CNNCifar, self).__init__()  # Corrected to reference CNNCifar
        self.layer1 = nn.Sequential(
            nn.Conv2d(3, 16, kernel_size=5, padding=2),  # Adjusted for 3 input channels
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.MaxPool2d(2))
        self.layer2 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.MaxPool2d(2))
        # Adjusted the size for CIFAR-10 images (32x32) after two MaxPool2d layers
        self.fc = nn.Linear(8 * 8 * 32, 10)  # Adjusted for CIFAR-10's 32x32 images, not 28x28 like MNIST
